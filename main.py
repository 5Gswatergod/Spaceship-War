...
pygame.display.set_caption("讓我看看")
...
font_name = os.path.join("Han.ttf")
...
def draw_init():
    draw_text(screen, '星際太空船', 72, WIDE/2, HEIGHT/4)
    draw_text(screen, 'A D 移動飛船 空白鍵發射子彈', 34, WIDE/2, HEIGHT/2)
    draw_text(screen, '按任意鍵開始遊戲', 24, WIDE/2, HEIGHT*3/4)
...
class Player(pygame.sprite.Sprite):
    ...
    def shoot(self):
        if not(self.hidden):
            if self.gun == 1:
                ...
                #bullet2 = Bullet(self.rect.centerx, self.rect.top)
                #bullet3 = Bullet(self.rect.centerx, self.rect.top)
                #bullet4 = Bullet(self.rect.centerx, self.rect.top)
                #all_sprites.add(bullet)
                #bullets.add(bullet)
                #shoot_sound.play()
            #elif self.gun >= 5:
    ...
    def hide(self):
        ...
        self.rect.center = (WIDE/ 2, HEIGHT+500)
...
class Rock(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.total_degree = 0
        self.rot_degree = random.randrange(1, 3)
...
while running:
    ...
    #畫面顯示
    screen.fill((79,94,126))
    screen.blit(background_img, (0, 0))
    all_sprites.draw(screen)
    draw_text(screen, str(score), 18, WIDE/2, 10)
    draw_health(screen, player.health, 5, 15)
    draw_lives(screen, player.lives, player_mini_img, WIDE - 100, 15)
    pygame.display.update()
